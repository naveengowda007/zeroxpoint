...
router.post('/getPrice', verifyToken, async (req, res) => {
	const { userid, cartid, CartData, PrintingInstructions, DeliveryLocation } = req.body
	...
	fs.readdir(filePath, async (err, files) => {
		...
		for (let index = 0; index < files.length; index++) {
			...
			const itemIndex = Cart.findIndex(item => item.uri.endsWith(file) || file === item?.name);
			if (itemIndex === -1) {
			...
			let CurPageCost = 0
			const CurPaperSize = Prices.PaperSizeData.find(CurPapaer => CurPapaer.name === item.size.name && CurPapaer.paperType.id === item.paper.id)
			...
			let bindingCost = Prices.bindingOptions.find(bind => bind.id === item.binding.id).price
			...
			if (['jpg', 'jpeg', 'heic', 'gif', 'png'].includes(fileExtension)) {
				...
				try {
					...
					console.log(error);
					return res.status(500).send({ message: 'Error extracting page count' })
				}
			}
				...
				try {
					const result = await mammoth.extractRawText({ path: currentFilePath });
				...
				try {
					...
					console.log(error);
					return res.status(500).send({ message: 'Error estimating page count' });
				}
			}
