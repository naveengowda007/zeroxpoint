...
const { generateInsertQuery, differenceInDays, generateUpdateQuery } = require('../zconfig/res');
const { verifyToken, verifyAdminToken, checkIfAdmin } = require('../zconfig/jwt');
...
router.post('/getOrder/:id', verifyToken, async (req, res) => {
	...
	const user = (await db(`SELECT name, phone, email FROM users WHERE userid = ?`, [userid]))[0];
	...
	for (let index = 0; index < shop_id.length; index++) {
		...
		let curShop = await db(`SELECT id, phone, shop_name, address_text, latitude, longitude FROM shops WHERE id = ?`, [id]);
		curShop = curShop[0]
		shop.push(curShop)
	}
	...
	if (differenceInDays(created_at) > 2 || (Order.status.length > 0 && Order.status.every(val => val === 'delivered'))) {
...
router.post('/getOrders', verifyAdminToken, async (req, res) => {
	...
	try {
		const Orders = [];
		const orders = await db(`SELECT * FROM orders ORDER BY id DESC LIMIT ${RowsPerPage} OFFSET ${offset}`);

		for (let i = 0; i < orders.length; i++) {
			let order = orders[i];
			let { userid, shop_id } = order;

			let user = await db(`SELECT userid, name FROM users WHERE userid = ?`, [userid]);
			user = user[0]
			...
			for (let index = 0; index < shop_id.length; index++) {
				...
				let curShop = (await db(`SELECT shop_name FROM shops WHERE id = ?`, [id]))[0];
				shop.push(curShop)
			}
...
router.post('/getOrders4Vendor', verifyToken, async (req, res) => {
	...
	try {
		const orders = await db(`SELECT id, cartid, created_at, shop_id, order_details, status FROM orders WHERE JSON_CONTAINS(shop_id, '?') ORDER BY id DESC LIMIT ${RowsPerPage} OFFSET ${offset}`, [user.id]);

		for (let index = 0; index < orders.length; index++) {
			const order = orders[index];
			const cart = (await db('SELECT * FROM cart WHERE cartid = ?', [order.cartid]))[0];
			order.filesCount = cart?.order_details?.cart?.length
		}

		// console.log(orders);
		const count = await db(`SELECT COUNT(*) FROM orders WHERE JSON_CONTAINS(shop_id, '?')`, [user.id]);
		const totalPages = Math.ceil(count[0]['COUNT(*)'] / RowsPerPage);
...
router.post('/downloadCart/:id', verifyToken, async (req, res) => {
	...
	const order = (await db(`SELECT * FROM orders WHERE id = ?`, [downloadId]))[0];
	if (!order)
		return res.status(403).send({ message: 'Something went wrong' })
	...
	for (let index = 0; index < shop_id.length; index++) {
		const id = shop_id[index];
		let curShop = await db(`SELECT id, userid, phone, shop_name, address_text, latitude, longitude FROM shops WHERE id = ?`, [id]);
		curShop = curShop[0]
		shop.push(curShop)
	}
	...
	zipFolder(folderPath, zipFilePath)
		.then(() => {
			res.send({ link: `https://zeroxpoint.com/adminapi/downloadZip/${outputName}` })
		})
		.catch((err) => {
...
router.post('/getShops4Order', verifyAdminToken, async (req, res) => {
	const vendors = await db(`SELECT id, vendor_name as name, phone, email, shop_name, address_text, latitude, longitude, default_payment_id FROM shops WHERE is_deleted = 0 AND
	userid IN (SELECT DISTINCT userid FROM vendorpricetable)`, [req.user.userid]);
...
router.post('/addShopToOrder', verifyAdminToken, async (req, res) => {
	...
	if (!order)
		return res.status(500).send({ message: 'No orders found' });
	...
	const updateSql = await generateUpdateQuery('orders', { shop_id, status, id: order.id }, "id")
...
router.post('/deliverOrder', verifyAdminToken, async (req, res) => {
	...
	const updateSql = await generateUpdateQuery('orders', { status, id: order.id }, "id")
	const result = await db(updateSql.sql, updateSql.fields);
...
router.post('/cancelOrder', verifyAdminToken, async (req, res) => {
	...
	if (order.status.length > 0 && order.status.every(item => item === 'delivered')) {
	...
	const updateSql = await generateUpdateQuery('orders', { order_details, id }, "id")
	const result = await db(updateSql.sql, updateSql.fields);
